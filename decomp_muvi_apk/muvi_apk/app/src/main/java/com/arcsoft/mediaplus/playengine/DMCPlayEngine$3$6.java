    ...
    implements Runnable
{
    ...
    final val.renderudn this$1;
    final int val$errorcode;
    final com.arcsoft.adk.atv.nGetTransportInfo val$info;
    final String val$renderudn;

    public void run()
    {
        Log.v("DMCPlay", (new StringBuilder()).append("GetTransInfo Status = ").append(val$info.strCurrentTransportState).append(" CurStatus = ").append(DMCPlayEngine.access$400(_fld0).getStatus()).toString());
        if (DMCPlayEngine.access$600(_fld0, cMsgType.MSG_GETACTION, val$renderudn))
        {
            if (val$errorcode != 0)
            {
                Log.v("DMCPlay", (new StringBuilder()).append("GetTransInfo errorcode=").append(val$errorcode).toString());
                return;
            }
            if (val$info.strCurrentTransportState == null)
            {
                Log.v("DMCPlay", "GetTransInfo strCurrentTransportState is null");
                return;
            }
            if (val$info.strCurrentTransportState.equalsIgnoreCase("PLAYING"))
            {
                RenderPlayingStatus(val$errorcode, val$info, val$renderudn);
                return;
            }
            if (val$info.strCurrentTransportState.equalsIgnoreCase("PAUSED_PLAYBACK"))
            {
                RenderPausedStatus(val$errorcode, val$info, val$renderudn);
                return;
            }
            if (val$info.strCurrentTransportState.equalsIgnoreCase("STOPPED"))
            {
                RenderStopedStatus(val$errorcode, val$info, val$renderudn);
                return;
            }
            if (val$info.strCurrentTransportState.equalsIgnoreCase("NO_MEDIA_PRESENT"))
            {
                RenderNoMediaStatus(val$errorcode, val$info, val$renderudn);
                return;
            }
        }
    }
...
/* anonymous class */
        ...
        implements com.arcsoft.adk.atv.RenderManager.IRenderPlayListener
    {
        ...
        public void onGetMute(final int errorcode, final boolean mute, final String renderudn)
        {
            DMCPlayEngine.access$300(DMCPlayEngine.this, new DMCPlayEngine._cls3._cls3());
        }
        ...
        public void onGetTransportInfo(int i, final com.arcsoft.adk.atv.MRCPCallback.DataOnGetTransportInfo info, final String renderudn)
        {
            ...
            DMCPlayEngine.access$300(DMCPlayEngine.this, i. new DMCPlayEngine._cls3._cls6());
        }
...
/* anonymous class */
        ...
        // Unreferenced inner class com/arcsoft/mediaplus/playengine/DMCPlayEngine$3$3

/* anonymous class */
        class DMCPlayEngine._cls3._cls3
            implements Runnable
        {
            ...
            public void run()
            {
                if (DMCPlayEngine.access$600(this$0, DMCPlayEngine.ASyncMsgType.MSG_GETMUTE, renderudn) && errorcode == 0 && DMCPlayEngine.access$1000(this$0) != mute)
                {
                    DMCPlayEngine.access$1002(this$0, mute);
                    if (DMCPlayEngine.access$500(this$0) != null)
                    {
                        DMCPlayEngine.access$500(this$0).OnMute(mute);
                        return;
                    }
                }
            }
