    ...
    implements Runnable
{
    ...
    public void run()
    {
        if (!DMCPlayEngine.access$1600(_fld0, cMsgType.MSG_PLAY, val$renderudn, null))
        {
            return;
        }
        Log.w("DMCPlay", (new StringBuilder()).append("onOpenMedia (get playing status) = ").append(val$errorcode).toString());
        DMCPlayEngineStatusMachine dmcplayenginestatusmachine = DMCPlayEngine.access$400(_fld0);
        sMachine.TransformAction transformaction = sMachine.TransformAction.PLAY;
        boolean flag;
        if (val$errorcode == 0)
        {
            flag = true;
...
/* anonymous class */
        ...
        implements com.arcsoft.adk.atv.RenderManager.IRenderPlayListener
    {
        ...
        private void onRenderPlayingStatus(int i, com.arcsoft.adk.atv.MRCPCallback.DataOnGetTransportInfo dataongettransportinfo, final String renderudn)
        {
            ...
            {
                DMCPlayEngine.access$1800(DMCPlayEngine.this, i. new DMCPlayEngine._cls3._cls7(), 500L);
            } else
            if (DMCPlayEngine.access$400(DMCPlayEngine.this).getStatus() != DMCPlayEngineStatusMachine.Status.SEEKING)
            {
